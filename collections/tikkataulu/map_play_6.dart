///   hkl  8.7.2015  edu/bin  0.0.6 -version of tikkataulu
...
///
///
/// TODO:   make  this  use   Server - Client
/// TODO:   or:    Use  HTML-page
///
//  Problems:  putIfAbsent do not work .. as  expected
//       teamed players are still adding throwCount
//  DIFFICULT TO team LAST   3 PLAYERS  Make them to form their own team
//  PlayersM   value-field  stays empty-valued, in 0,  myistake lol
//  Trying to use   playersM.forEach(...  in throw-Round   NO
//  added  totalThrows   count
//  DO better way to add to teams.  Is there??
//  BUT:   some players are not in teams     OK

//  Use temporary Players Map yep.
//  =  do not allow same player to be many times in teams   OK
...
void doThrowRound(int howMany) {              //  reserving parameter for future usage
  ...
  //  lol  changed _i  from  var  to int  for to make boolean work (?)
  for (int _i = 1; _i < playersCount+1; _i++) {

    nowThrowing = _i;
    int nowResult = keysFreqL[random.nextInt(keysFreqL.length)];    //  one of   106
    totalThrows ++;                                      // + also teamed Players

    //  separated two nested if clause here
    //  must assume: containsValue can not seek <int,List<int>>
    // if (!teamsM.containsValue(nowThrowing)) {
      //  lol,   do not work
      //  Now I done my job I'm going home.. and I...
      if (!teamedPlayersL.contains(nowThrowing)) {
        print('///////////////////////////////    player $nowThrowing is not yet teamed');
        addToTeam(nowResult, nowThrowing);
      //  yes this do not handle all cases
      } else  {
                      //  empty clause
      };
    //}      //  away
